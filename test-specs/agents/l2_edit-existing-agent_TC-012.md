...
| Agent overflow (⋮) menu button | `page.locator('#undefined-action')` — **flag**: this is the element's literal live DOM `id`, not a placeholder in this doc; a broken template interpolation in the product's menu-button component, confirmed on multiple agent ids (253, 271, 273) — see [GH#33](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/33) (canonical) for the full ARIA-wiring writeup (id-collision risk, dialog's broken `aria-labelledby`, unnamed confirm-input) | position-based: the icon-only button following Save/Save As Version/Discard in the detail-page toolbar |
...
- **3 case-authoring clarifications filed as [GH#34](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/34)** (bundled per the project's existing case-text-drift convention — see GH#9, #10, #12, #14, #28, #30):
  ...
  4. The overflow-menu button's `id="undefined-action"` is more thoroughly documented under [GH#33](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/33) (canonical; originally filed from TC-016, corroborated by TC-013/TC-012) — see § Concrete Handles above. (Note: an earlier version of this AFS cited #39 for this, which was itself closed as a duplicate of #33 shortly after — corrected here.)
