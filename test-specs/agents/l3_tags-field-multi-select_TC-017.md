...
- Step 16 (chip removal) — *added entirely: the original case has no removal step at all. Per this analyst's dispatch brief ("deep dive: add multiple tags, remove one, verify chip rendering"), this is required scope, not scope creep. Discovered the chip's own `button` wrapper is the full click target for removal (no need to target a nested icon specifically) — a materially simpler automation handle than assuming icon-only removal.* **[Implementer correction, 2026-07-02]**: this claim did NOT reproduce during automation Phase 2 exploration — clicking the outer chip element (a MUI `<div role="button" class="MuiChip-root ...">`) was confirmed live to do nothing, repeatedly. The chip actually wraps a separate `<svg class="MuiChip-deleteIcon">` that MUI wires `onDelete` to specifically; clicking that SVG removes the chip reliably on the first attempt every time. `tests/pages/agentForm.page.ts`'s `removeTag()` targets `.MuiChip-deleteIcon` directly. Reverse-masking guard: the live component structure is ground truth here, the AFS's untested claim was wrong.*
...
| Tag chip (after add) | `page.getByRole('button', { name: tagName, exact: true })` scoped to the Tags field's container, for visibility/presence assertions | `.MuiChip-root:has-text(tagName)` |
| Tag chip delete icon | **[Implementer correction, 2026-07-02]** `page.getByRole('button', { name: tagName, exact: true }).locator('.MuiChip-deleteIcon')` — clicking the outer chip element does NOT remove it (confirmed live, repeatedly); the chip's own `<svg class="MuiChip-deleteIcon">` is the actual `onDelete` trigger. This AFS's original claim ("the whole button is the delete trigger") did not reproduce. | none needed |
...
| Detail page three-dot menu button | **No accessible name** — Playwright resolves it as `page.locator('#undefined-action')` (the app's own id template appears to concatenate `${x}-action` with `x` evaluating to `undefined` — already filed as [`GH#33`](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/33) by the TC-016 analyst; cosmetic, not blocking) | position-based: last icon-only `button` in the version-toolbar row |
...
- **[Implementer amendment, 2026-07-02] — technique finding, not filed as a defect**: step 8's live-suggestion `listbox`/`option` for a pre-existing tag ("automation") did **not** reproduce during implementer Phase 2 exploration, using either `.fill()` or character-by-character `page.keyboard.type()` — confirmed via direct DOM inspection that the combobox's own `aria-expanded` stayed `"false"` and no popper/listbox element existed anywhere in the DOM in either attempt, despite the input holding the correct value ("automation") and `aria-autocomplete="list"` confirming the control supports it. This single AFS-documented observation (one analyst session, one occurrence) did not reproduce across two independent automated attempts — consistent with either a rare/flaky trigger condition or a session-specific artifact, not a reliably-automatable behavior. Per the reverse-masking guard and this being Axis-2 "additional" coverage (not required by the original case's own Coverage Map — the case's actual requirement is "tag becomes a chip after Enter", which DOES reproduce reliably), `tests/agents.spec.ts` treats the suggestion-listbox check as best-effort (does not fail the test if absent) rather than a hard assertion, and still hard-asserts the chip appearing after Enter (the real, load-bearing behavior).
