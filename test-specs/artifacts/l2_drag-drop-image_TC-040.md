...
- **Third amendment (2026-07-09, correction — supersedes the second amendment's residual-defect claim and GH#124)**: the "counter still stuck even with the single-gesture fix" observation does NOT reproduce in pristine contexts. Re-verified two independent ways: (1) a from-first-principles probe in a fresh browser session using the exact same synthesized-`DataTransfer` single-gesture technique — counter decremented "10 left" → "9 left" immediately after the drop (and → "8 left" after a second file via the file input), with no `dragleave`/`dragend` or re-render nudge needed; (2) this suite's own TC-040 run passed end-to-end including the `/9 left/` assertion. The 2026-07-03 observation most likely came from the debugging session's own earlier unbalanced experimental gestures poisoning the app's enter/leave bookkeeping for the rest of that page's lifetime — the same failure mode the second amendment had just root-caused, observed one level up: the *verification* of the fix ran in a page the experiments had already broken. A defect first observed via synthetic event dispatch must be reproduced in a fresh, isolated context (single complete gesture) before filing — see the suite's pristine-repro discipline. GH#124 closed as not reproducible; the spec's soft-assert reverted to a hard assert.
