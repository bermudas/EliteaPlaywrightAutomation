...
| 8 Verify "Name" field still contains `TEST_` | form data preserved during modal appearance | step 8 | step 8: value read-back in the original (untouched) tab | asserted (soft — known intermittent defect [`GH#72`](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/72), ~20% observed rate; see § Known Defects) |
...
| 11 Verify Name empty OR contains `TEST_` | form state reflects expected behavior | step 11 | step 11: value read-back — resolves to the "contains `TEST_`" branch under the new-tab variant (both branches individually confirmed reachable, see step 6 note) | asserted (soft — known intermittent defect [`GH#72`](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/72); see § Known Defects) |
...
- **[MINOR/DEFECT]** Filed as [`GH#72`](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/72) — implementer re-investigation (PR #70 review round 1, 2026-07-03) found the new-tab variant's own confirmed-live claim ("original tab's React component tree is completely unaffected by navigation in an unrelated tab") does NOT hold up under more runs (~20% observed rate): the "Name" field intermittently, genuinely clears (a settled empty value, not a slow-render race) in the original tab shortly after the second tab performs its own navigation/bootstrap. Ruled out `document.visibilitychange`/`window.blur` as the mechanism via direct instrumentation; confirmed the app maintains multiple concurrent Socket.IO/WebSocket connections per tab, consistent with (but not proven to be) a client-side race tied to the shared session's real-time/background activity. Confirmed independent of this batch's other two findings (GH#71 — `createFixture()`'s own composer-targeting race) since TC-056 never exercises that code path. Steps 8 and 11 downgraded to `expect.soft()` per the isolated-step-defect handling rule — the rest of the flow (re-fill, save, verify creation) still runs and passes regardless.
