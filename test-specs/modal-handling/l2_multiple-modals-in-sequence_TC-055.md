...
- **[MINOR/DEFECT]** Filed as [`GH#71`](https://github.com/bermudas/EliteaPlaywrightAutomation/issues/71) — implementer re-investigation (PR #70 review round 1, 2026-07-03): the disposable-fixture creation helper (`ConversationPage.createFixture()`, shared with TC-052) relies on the sidebar "+ Conversation" control atomically binding the composer to a genuinely blank conversation. Confirmed live this is NOT atomic — for a brief window (up to ~1s) after the click, the composer can still target whichever conversation was previously active, silently sending the fixture message into that pre-existing conversation instead of creating a new one. This was the actual root cause of the "5 leaked conversations" flagged in PR #70 round-1 review (none were real separate conversations — all were message pairs inside the pre-existing `smoke.spec.ts` "Hello, test" conversation, confirmed via direct API cross-check). `createFixture()` now waits for the message-thread panel to empty before interacting with the composer, eliminating the race for automation purposes; the underlying product behavior (composer not atomically re-bound) remains open against GH#71.
