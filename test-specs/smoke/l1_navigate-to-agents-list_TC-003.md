...
**Total agent count (footer)** — this AFS's own "Recommended Locator" is `page.getByText('Agents:').locator('xpath=following-sibling::*[1]')`. Re-verified live (2026-07-02): the label and count ARE separate sibling `<span>`s inside a shared parent (`<span class="MuiTypography-...">`Agents:`</span><span>`211`</span>...`), so the AFS's own xpath locator is structurally accurate and would work. The implementation (`tests/smoke.spec.ts`) instead uses `page.getByText(/^Agents:\s*\d+/)`, which matches at the shared-parent level (Playwright's text matching includes descendant text, and the regex tolerates the concatenated `"Agents:211Published:1..."` content), then extracts the count via regex on that matched element's own text rather than isolating the count span through an xpath traversal. Chosen per `.agents/testing.md` § Locator strategy's ladder, which ranks a text-content match above CSS/XPath structural traversal (xpath is explicitly the ladder's last-resort tier) — not because the AFS's own locator was wrong, but because this avoids the lower-tier technique when a higher-tier one is available and confirmed working (3 consecutive green runs). Not a correctness bug — flagged here per the standing AFS-drift documentation requirement, since it wasn't called out in this AFS's original Concrete Handles table.
